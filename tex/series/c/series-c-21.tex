...
\begin{checkboxes}
    \choice \cinline{fopen}
    \choice \cinline{fgets}
    \CorrectChoice \cinline{fseek}
    \choice \cinline{fputc}
    \choice \cinline{feof}
\end{checkboxes}

\part Quel est le mode à transmettre à l'appel \cinline{fopen("f.txt", mode)} pour ouvrir un fichier existant en mode binaire en lecture écriture ?
\begin{checkboxes}
    \choice \cinline{"r"}
    \choice \cinline{"w"}
    \CorrectChoice \cinline{"ab+"}
    \choice \cinline{"aw"}
    \choice \cinline{"w+"}
\end{checkboxes}
\end{parts}

\question Vous disposez d'un pointeur sur un fichier ouvert en lecture \cinline{fp} et vous souhaitez connaître la taille de ce fichier. Écrire une fonction \cinline{size_t fsize(FILE *fp)} qui retourne la taille du fichier.
